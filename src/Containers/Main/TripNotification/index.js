...
const TripNotification = (props) => {
  let dist, time, place, cLatitude = 0, cLongitude = 0, ride_type, restaurantID, restaurantCompare, distDrop, timeDrop, placeDrop;
  ...
  const watchingPosition = () => {
    ...
    const calculateDistanceTime = (latitude, longitude) => {
      axios({
        method: 'get',
        url:
          'https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=' + cLatitude + ',%20' + cLongitude + '&destinations=' + latitude + ',%20' + longitude + '&key=AIzaSyC6Vo_6ohnkLyGIw2IPmZka0TarRaeWJ2g',
      })
        .then(function (response) {
          console.log('google map api response: ', response);
          setDistance(response?.data?.rows[0]?.elements[0]?.distance?.value * 0.000621371192),
    ...
    const calculateDropOffDistanceTime = (picklat, picklng, droplat, droplng) => {
      axios({
        method: 'get',
        url:
          'https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=' + picklat + ',%20' + picklng + '&destinations=' + droplat + ',%20' + droplng + '&key=AIzaSyC6Vo_6ohnkLyGIw2IPmZka0TarRaeWJ2g',
      })
        .then(function (response) {
  ...
  <View style={Layout.fill}>
    ...
    {pickupCoordinates.length > 0 ?
      ...
      {headers && 
      <BottomHeader
        onPressAccept={acceptRide}
        onPressReject={cancelRide}
      />}
  </View>
  )
}
