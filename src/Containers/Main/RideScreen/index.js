...
import { View, Text, StyleSheet, Dimensions, PermissionsAndroid, Platform, DeviceEventEmitter,
} from 'react-native';
...
const RideScreen = (props) => {
  ...
  const calculateDistanceTime = (latitude, longitude) => {
    if (locationSubChildKey) {
      ...
      console.log('LAT LONG IN calculate,', latitude, longitude, "pickup's", pickupLocation[0]?.latitude, pickupLocation[0]?.longitude );
    ...
    axios({
      ...
      .then(function (response) {
        ...
        // that.setState({
          setDistance(response?.data?.rows[0]?.elements[0]?.distance?.value * 0.000621371192)?.toFixed(2),
          setTime(response?.data?.rows[0]?.elements[0]?.duration?.value / 60)?.toFixed(0)
