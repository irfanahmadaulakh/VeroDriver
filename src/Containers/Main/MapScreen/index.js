...
const MapScreen = () => {
  ...
  const getGasStations = ()=> {
    axios({
      method: 'get',
      url: 
      'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=' + driverLocation?.latitude + ',' + driverLocation?.longitude + '&radius=5000&type=gas_station&key=AIzaSyAAB7FFI7kfgYE97wOqnP_CxCDzWkXtV_w',
    })
  ...
  return (
    ...
    {console.log("Station",gasStationMarker)}
      ...
      gasStationMarker.map((item, index)=>(
        <Marker
          ...
          >
            <Image source={{uri: item?.icon}} style={{ width: WP('5'), height: WP('5')}} />
          </Marker>
      ))
      }
    </MapView>}
    ...
    <GasStationButton onGassPress={()=>console.warn("No gas stations nearby")}/>
    <Earnings/>
